...
### 문제 상황
외부 기관 API는 동기 응답 방식이지만, **성공 시 응답을 매우 오래 물고 있는(hanging) 특성**이 있습니다.

**외부 기관 응답 패턴:**
- ✅ **2차 인증 필요**: 즉시 응답 (200ms 이내)
- ✅ **발급 실패**: 즉시 응답 (200ms 이내)
- ❌ **발급 성공**: 응답을 오래 물고 있음 (200ms~ 이상, 동기로 대기)

**핵심:**
- 실패/2차 인증은 **즉시 응답**이 옴
- 성공은 **응답이 없는 게 아니라** 응답을 매우 오래 물고 있음
- 따라서 "일정 시간 응답이 안 오면 = 성공"으로 판단
